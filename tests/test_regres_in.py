...
def test_create_user(base_url):
    ...
    response = requests.post(url=f"{base_url}/api/users", data=payload)
...
def test_update_user(base_url):
    ...
    response = requests.put(url=f"{base_url}/api/users/2", data=payload)
...
def test_register_user_unsuccessful(base_url):
    payload = {
        "email": "test@example.com"
    }

    response = requests.post(url=f"{base_url}/api/register", data=payload)
...
def test_login_user(base_url):
    ...
    response = requests.post(url=f"{base_url}/api/login", data=payload)
